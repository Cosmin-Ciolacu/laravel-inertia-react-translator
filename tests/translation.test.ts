...
const dictionary = {
  ...
  companies: {
    ...
    title:
      "{0}No company available|{1} One company available|[2-10] :count companies available|[11-*] A lot of companies available",
...
const __ = (key: string, replace?: ReplaceObject) =>
...
test("it can translate a string with placeholders", () => {
  ...
  expect(__("greetings.hello", { name: "Jane", other: "Sam" })).toBe(
    "Hello Jane"
  );
...
test("it can translate and replace a key with a missing translation", () => {
  expect(__("Hi, my name is :name", { name: "John" })).toBe(
    "Hi, my name is John"
  );
});
...
test("it can translate a composite key", () => {
  ...
  expect(__("companies.name[2]{name:Apple,other:Microsoft}")).toBe(
    "Company Apple and Microsoft"
  );
  ...
  expect(__("companies.name[2]{name:Apple")).toBe(
    "companies.name[2]{name:Apple"
  );
